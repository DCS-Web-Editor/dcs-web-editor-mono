...
const component: Component = {
  ...
  render: (c: Context) => {
    ...
    const visibility = Math.round(
      calculator.distance(weather.visibility.distance / 1000)
    ).toLocaleString();
    ...
    return (
      title +
      `<span class="label">QNH</span> ${pressure}${pressureUnit} <span class="label">TEMP</span> ${weather.season.temperature.toFixed(
        1
      )}°C ${toFahrenheit(weather.season.temperature).toFixed(
        1
      )}°F <span class="label">VISIBILITY</span> ${
        visibility + dunit
      } <span class="label">CLOUD</span> <span class="label">BASE</span> <span class="value">${
        base + aunit
      }</span> <span class="label">THICKNESS</span> ${thickness + aunit}
