...
const component: Component = {
  ...
  control: `
  <label>Units</label>
  <select name="metricSelect" id="metric-select-control" title="Warning: changing units will reset user input">
  ${options.map((o) => {
    return `<option value="${o.value}" ${calculator.config.system === o.value ? "selected" : ""}>${
      o.label
    }</option>`;
  })}
</select>`,
