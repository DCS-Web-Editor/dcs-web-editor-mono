...
const component: Component = {
  id: 'download-all',
  template: false,
  control: `<div id="download-all" class="button-group">
  <button id="download-all-button" title="Download PDF document for each Client in coalition. May take some time...">Download all PDF</button>
  <select name="coalitionSelect" id="download-select-coalition" title="Download all PDF for coalition">
    ${
      options.map(o => {
        return `<option value="${o.value}" ${coalitionName === o.value ? 'selected' : ''}>${o.label}</option>`
      })
    }
</select>
  <select name="typeSelect" id="download-select-category" title="Download all PDF for category">
    ${
      categoryOptions.map(o => {
        return `<option value="${o.value}" ${category === o.value ? 'selected' : ''}>${o.label}</option>`
      })
    }
</select>
</div>
      ...
      for (const country of countries) {
        ...
        for (const group of groups) {
          ...
          for (const unit of units) {
            ...
            await timeout(500);
            renderKneeboard(unit.name, group.name, category, country.name, coalitionName, mission, dictionary, {noControls: true});
            await timeout(500);
            makePdf();
            await timeout(3000);
          }
