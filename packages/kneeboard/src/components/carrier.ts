  ...
  render: (c: Context) => {
    ...
    const carriers = `<ul>${country.ship?.group.map((group) => {
      ...
      if (group.route) {
        ...
        // TACAN
        const tacanTasks = group.route?.points?.flatMap(p => p.task.params?.tasks?.filter(t => t.id === 'WrappedAction' )).filter(t => t.params?.action?.id === 'ActivateBeacon').map(t =>  t.params?.action?.params)
        const tacanTask = tacanTasks[0];
        const tacan = tacanTask ? `<span class="tacan">TACAN ${tacanTask.callsign} ${tacanTask.channel}${tacanTask.modeChannel}</span>` : '';
        
        // ICLS
        const iclsTasks = group.route?.points?.flatMap(p => p.task.params?.tasks?.filter(t => t.id === 'WrappedAction' )).filter(t => t.params?.action?.id === 'ActivateICLS').map(t =>  t.params?.action?.params)
